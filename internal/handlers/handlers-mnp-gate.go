...
func NewMnpGatePageHandler(db *sql.DB, tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		rows, err := db.Query("SELECT id, engine, max_workers, cache_days, config FROM mnp_gate_config")
		if err != nil {
			// If table missing, render empty page
			if rows == nil {
				tmpl.ExecuteTemplate(w, "mnp_gate", map[string]interface{}{"Configs": []interface{}{}})
				return
			}
			http.Error(w, "Database query error: "+err.Error(), http.StatusInternalServerError)
			return
		...
		if err := tmpl.ExecuteTemplate(w, "mnp_gate", data); err != nil {
...
func NewMnpGatesListHandler(db *sql.DB, tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			tmpl.ExecuteTemplate(w, "mnp_gates", map[string]interface{}{"Gates": []interface{}{}})
			return
		}
		defer rows.Close()
		...
		if err := tmpl.ExecuteTemplate(w, "mnp_gates", data); err != nil {
			http.Error(w, "Template render error: "+err.Error(), http.StatusInternalServerError)
			return
		}
	}

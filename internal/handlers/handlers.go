...
// HomeHandler holds dependencies for the home page handler
type HomeHandler struct {
	DB   *sql.DB
	Tmpl *template.Template
}

// NewHomeHandler constructs a HomeHandler and returns it as http.Handler
func NewHomeHandler(db *sql.DB, tmpl *template.Template) http.Handler {
	return &HomeHandler{DB: db, Tmpl: tmpl}
}
...
func (h *HomeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Get DB version
	if err := h.DB.QueryRow("SELECT VERSION()").Scan(&data.Version); err != nil {
